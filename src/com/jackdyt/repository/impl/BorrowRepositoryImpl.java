...
public class BorrowRepositoryImpl implements BorrowRepository {
    ...
    public List<Borrow> findAllByReaderId(Integer id, Integer index, Integer limit) {
        Connection connection = JDBCTools.getConnection();
        String sql =
                //"select br.id,b.name,b.author,b.publish,br.borrowtime,br.returntime,r.name,r.tel,r.cardid,br.state from borrow br,book b,reader r where readerid = ? and b.id = br.bookid and r.id = br.readerid limit ?,?";
                "select borrow.id,book.name,book.author,book.publish,borrow.borrowtime,borrow.returntime,reader.name,reader.tel,reader.cardid,borrow.state from borrow, book, reader where readerid = ? and book.id = borrow.bookid and reader.id = borrow.readerid limt ?,?";
        PreparedStatement preparedStatement = null;
        ...
        try {
            ...
            while(resultSet.next()){
                borrowlist.add(new Borrow(resultSet.getInt(1),new Book(resultSet.getString(2),resultSet.getString(3),resultSet.getString(4)),
    ...
    public int count(Integer id) {
        Connection connection = JDBCTools.getConnection();
        String sql ="select count(*) from book, borrow,reader " +
                "where readerid = ? and book.id = borrow.bookid and reader.id = borrow.readerid ";
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        int count = 0;
        try {
    ...
    public int coutByState(Integer state) {
        Connection connection = JDBCTools.getConnection();
        String sql ="select count(*) from book, borrow,reader " +
                "where state = ? and book.id = borrow.bookid and reader.id = borrow.readerid ";
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        int count = 0;
        try {
